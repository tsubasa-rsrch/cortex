"""Cortex Replay: Real-World Data Pipeline.

Replays real events from a daemon session through the Cortex perception
pipeline to demonstrate cognitive filtering (habituation, orienting
responses, circadian patterns).
"""
...
def load_events(path: str = None) -> list:
    ...
    2. ~/.tsubasa-daemon/memory/event_log.jsonl (live daemon data)
    3. Bundled sample data in cortex/data/sample_events.jsonl
    4. Examples directory sample_events.jsonl
    """
    if path is None:
        candidates = [
        ...
        events.append(
            {
                ...
                "metadata": {
                    ...
                    "urgency": (
                        "urgent"
                        if diff >= 30
                        else "high" if diff >= 20 else "normal"
                    ),
                },
            }
        )
...
def replay(events: list, verbose: bool = False) -> dict:
    """Replay events through Cortex perception pipeline.

    Returns dict with stats: total_events, motion_events, passed,
    filtered, orienting, reduction_pct.
    """
    tmp = Path(tempfile.mkdtemp())

    hab = HabituationFilter(
        cooldown=30.0, base_threshold=15.0, orienting_mult=2.0
    )
    ...
    print(
        f"  {_bold('Telegram events:')} {_white(str(len(telegram_events)))}"
    )
    print(
        f"  {_bold('Other:')}           "
        f"{_white(str(total - len(motion_events) - len(telegram_events)))}"
    )
    print()
    ...
    for event in motion_events:
        ...
        camera_stats[camera]["max_diff"] = max(
            camera_stats[camera]["max_diff"], diff
        )
        if diff >= 30:
        ...
        if not should:
            ...
            if verbose:
                print(
                    f"  {_dim('[FILTERED]')} {camera} diff={diff:.1f}"
                    f" {_dim(f'({reason})')}"
                )
        else:
            passed += 1
            ...
            if is_orienting:
                ...
                if verbose:
                    print(
                        f"  {_bg_yellow(' ORIENTING ')} {camera}"
                        f" diff={diff:.1f}"
                        f" {_yellow('<- sudden change!')}"
                    )
    ...
    print(
        f"  {_bold('Passed (conscious):')}      "
        f"{_green(str(passed))} ({passed * 100 // max(motion_count, 1)}%)"
    )
    print(
        f"  {_bold('Filtered (habituated):')}   "
        f"{_red(str(filtered))} "
        f"({filtered * 100 // max(motion_count, 1)}%)"
    )
    print(f"  {_bold('Orienting responses:')}     {_yellow(str(orienting))}")
    print(
        f"  {_bold('Cognitive load reduction:')} "
        f"{_bg_green(f' {reduction:.0f}% ')}"
    )
    print()
    ...
    for cam, stats in sorted(camera_stats.items()):
        ...
        urgent_str = (
            _red(str(stats["urgent"])) if stats["urgent"] > 0 else "0"
        )
        print(
            f"    {_cyan(cam)}: {stats['count']} events, avg_diff={avg:.1f}, "
    ...
    print(
        f"  {_white(str(motion_count))} raw stimuli"
        f" {_cyan('->')} {_green(str(passed))} conscious events"
    )
